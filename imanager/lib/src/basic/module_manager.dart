...
class ModuleManager implements IEvent{
  ...
  /// lifecycle of all modules: pre_init -> post_init -> inited
  void init() async {
    // init high priority modules --- for environment / config / sdk / etc
    moduleList.values.where( (IModule module) => module.getInitPriority() == Priority.high).forEach((IModule module) {
      module.init(this);
    });
    // init medium priority modules --- for business logic
    moduleList.values.where( (IModule module) => module.getInitPriority() == Priority.medium).forEach((IModule module) {
      module.init(this);
    });
    // init low priority modules --- for others 
    moduleList.values.where( (IModule module) => module.getInitPriority() == Priority.low).forEach((IModule module) {
      module.init(this);
    });
    // init none priority modules
    moduleList.values.where( (IModule module) => module.getInitPriority() == Priority.none).forEach((IModule module) {
      module.init(this);
    });
  }
